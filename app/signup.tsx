...
const STREAMING_SERVICES = [
  { id: "spotify",    name: "Spotify",       color: "#1DB954", iconType: "fa5",  icon: "spotify"    },
  { id: "apple",      name: "Apple Music",   color: "#FA233B", iconType: "fa5",  icon: "apple"      },
...
const DUMMY_ARTISTS = [
  { id: "a1", name: "Wizkid",     genre: "Afrobeats",  color: "#FF6B35", initials: "WK" },
  { id: "a2", name: "ROSÉ",       genre: "K-Pop",      color: "#AB00FF", initials: "RO" },
  { id: "a3", name: "Rema",       genre: "Afrobeats",  color: "#CAFF00", initials: "RE" },
  { id: "a4", name: "Bernadya",   genre: "Indie Pop",  color: "#FF3CAC", initials: "BD" },
  { id: "a5", name: "Ayra Starr", genre: "Afropop",    color: "#00C2FF", initials: "AS" },
  { id: "a6", name: "SZA",        genre: "R&B",        color: "#7B61FF", initials: "SZ" },
  { id: "a7", name: "Burna Boy",  genre: "Afrofusion", color: "#CAFF00", initials: "BB" },
  { id: "a8", name: "Olivia R.",  genre: "Pop",        color: "#FF6B35", initials: "OR" },
];

const STEP_TITLES = ["Email", "Strong Password", "Your Username", "Birthday", "Link Streaming", "Artists for You"];
const STEP_SUBS   = [
...
function Field({ placeholder, secureTextEntry, keyboardType, autoCapitalize, value, onChangeText }: {
  ...
  return (
    <View style={{ gap: 5 }}>
      <View style={{ flexDirection: "row", gap: 4 }}>
        {[0,1,2,3].map((i) => (
          <View key={i} style={{ flex: 1, height: 3, borderRadius: 2, backgroundColor: i < s ? COLORS[s-1] : "rgba(255,255,255,0.1)" }} />
        ))}
      </View>
      <Text style={{ fontSize: 11, color: COLORS[s-1], textAlign: "right", fontWeight: "700" }}>{LABELS[s-1]}</Text>
    </View>
  );
}
...
function PasswordField({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <View style={{ gap: 10 }}>
      <View style={[styles.field, { flexDirection: "row", alignItems: "center", paddingVertical: 0 }]}>
        <TextInput
          style={{ flex: 1, paddingVertical: 15, color: "#fff", fontSize: 15, fontWeight: "600" }}
        ...
        />
        <TouchableOpacity onPress={() => setShow((p) => !p)} activeOpacity={0.7} style={{ padding: 12 }}>
          <Ionicons name={show ? "eye-off-outline" : "eye-outline"} size={20} color="rgba(255,255,255,0.38)" />
        </TouchableOpacity>
      </View>
...
function UsernameField({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    <View style={[styles.field, { flexDirection: "row", alignItems: "center", paddingVertical: 0 }]}>
      <Text style={{ fontSize: 16, fontWeight: "900", color: "#AB00FF", paddingVertical: 15, paddingRight: 2 }}>@</Text>
      <TextInput
        style={{ flex: 1, paddingVertical: 15, paddingLeft: 4, color: "#fff", fontSize: 15, fontWeight: "600" }}
      ...
      />
      {value.length > 2 && (
        <Ionicons name="checkmark-circle" size={20} color="#00E5A0" style={{ padding: 12 }} />
      )}
    </View>
  );
}
...
function DrumPicker({ values, initialIndex = 0, onSelect, flex }: {
  values: string[]; initialIndex?: number;
  onSelect: (v: string) => void; flex?: number;
}) {
  ...
  useEffect(() => {
    setTimeout(() => {
      scrollRef.current?.scrollTo({ y: initialIndex * DRUM_H, animated: false });
    }, 80);
  }, []);

  const snap = (y: number) => {
    const idx = Math.max(0, Math.min(values.length - 1, Math.round(y / DRUM_H)));
    setSelIdx(idx);
    onSelect(values[idx]);
  };
  ...
  return (
    <View style={{ flex: flex ?? 1, height: DRUM_H * 3, overflow: "hidden" }}>
      <View pointerEvents="none" style={[StyleSheet.absoluteFill, { justifyContent: "center" }]}>
        <View style={{ height: DRUM_H, borderTopWidth: StyleSheet.hairlineWidth, borderBottomWidth: StyleSheet.hairlineWidth, borderColor: "rgba(171,0,255,0.65)" }} />
      </View>
      <ScrollView
        ref={scrollRef}
      ...
      >
        {values.map((v, i) => (
          <View key={v + i} style={{ height: DRUM_H, alignItems: "center", justifyContent: "center" }}>
            <Text style={{
              fontSize: i === selIdx ? 24 : 16,
              fontWeight: i === selIdx ? "800" : "400",
              color: i === selIdx ? "#fff" : Math.abs(i - selIdx) === 1 ? "rgba(255,255,255,0.28)" : "rgba(255,255,255,0.1)",
...
function BirthdayDrumPicker({ onChange }: { onChange: (date: string) => void }) {
  ...
  useEffect(() => { notify(monthRef.current, dayRef.current, yearRef.current); }, []);

  return (
    <View style={styles.drumWrap}>
      <DrumPicker values={MONTHS} initialIndex={0} flex={1.3}
        onSelect={(v) => { monthRef.current = v; notify(v, dayRef.current, yearRef.current); }} />
      <View style={styles.drumDivider} />
      <DrumPicker values={DAYS} initialIndex={0} flex={0.9}
        onSelect={(v) => { dayRef.current = v; notify(monthRef.current, v, yearRef.current); }} />
      <View style={styles.drumDivider} />
      <DrumPicker values={YEARS} initialIndex={0} flex={1.2}
        onSelect={(v) => { yearRef.current = v; notify(monthRef.current, dayRef.current, v); }} />
    </View>
  );
}
...
function StreamingGrid({ connected, onToggle }: { connected: Set<string>; onToggle: (id: string) => void }) {
  return (
    <View style={{ gap: 10 }}>
      {STREAMING_SERVICES.map((s) => {
        ...
        return (
          <TouchableOpacity
            key={s.id}
            style={[styles.streamRow, { backgroundColor: s.color }]}
          ...
          >
            ...
            <View style={styles.streamIconWrap}>
              {s.iconType === "fa5" ? (
                <FontAwesome5 name={s.icon} size={18} color="#000" />
              ) : (
            ...
            {ok ? (
              <View style={styles.streamConnectedPill}>
                ...
                <Text style={[styles.streamConnectedText, { color: s.color }]}>Connected</Text>
              </View>
            ) : (
...
function ArtistsGrid({ followed, onToggle }: { followed: Set<string>; onToggle: (id: string) => void }) {
  return (
    <View style={{ gap: 10 }}>
          ...
          {DUMMY_ARTISTS.slice(ri * 2, ri * 2 + 2).map((a) => {
            ...
            return (
              <View key={a.id} style={styles.artistCard}>
                <View style={[styles.artistAvatar, { backgroundColor: a.color + "20", borderColor: a.color + "55" }]}>
                  <Text style={[styles.artistInitials, { color: a.color }]}>{a.initials}</Text>
                </View>
                <Text style={styles.artistName} numberOfLines={1}>{a.name}</Text>
                <Text style={styles.artistGenre}>{a.genre}</Text>
                <TouchableOpacity
                  style={[styles.artistFollowBtn, isF && styles.artistFollowBtnActive]}
                  onPress={() => onToggle(a.id)}
                  activeOpacity={0.8}
                >
                  <Text style={[styles.artistFollowText, isF && styles.artistFollowTextActive]}>
...
export default function SignupScreen() {
  ...
  const [streaming, setStreaming] = useState<Set<string>>(new Set(["spotify"]));
  ...
  useEffect(() => {
    Animated.parallel([
      Animated.timing(contentFade, { toValue: 1, duration: 1000, delay: 1500, useNativeDriver: true }),
      Animated.timing(slideAnim,   { toValue: 0, duration: 50,   delay: 100, easing: Easing.out(Easing.exp), useNativeDriver: true }),
    ]).start();
  }, []);
  ...
  const fade = (cb: () => void) => {
    Animated.timing(switchFade, { toValue: 0, duration: 140, useNativeDriver: true }).start(() => {
      cb();
      Animated.sequence([
        Animated.delay(200),
  ...
  const openExpanded = (cb: () => void) => {
    cb();
    Animated.parallel([
      Animated.spring(expandY, { toValue: 0, useNativeDriver: true, damping: 22, stiffness: 190 }),
      Animated.timing(expandO, { toValue: 1, duration: 260, useNativeDriver: true }),
    ]).start();
  };

  const closeExpanded = (cb: () => void) => {
    Animated.parallel([
      Animated.timing(expandY, { toValue: SH * 0.3, duration: 240, easing: Easing.in(Easing.cubic), useNativeDriver: true }),
      Animated.timing(expandO, { toValue: 0, duration: 200, useNativeDriver: true }),
    ]).start(() => cb());
  };
  ...
  const switchMode = (next: Mode) =>
    fade(() => {
      ...
      expandO.setValue(0);
      cardBgProgress.value = withTiming(next === "idle" ? 0 : 1, { duration: 360 });
    });
  ...
  const toggleStreaming = (id: string) =>
    setStreaming((prev) => { const n = new Set(prev); n.has(id) ? n.delete(id) : n.add(id); return n; });
  ...
  const handleLogin = () => {
    loginBtnRef.current?.measure((_, __, w, h, pageX, pageY) => {
      ...
      Animated.timing(loginExpandScale, {
        toValue: 40, duration: 550, easing: Easing.in(Easing.exp), useNativeDriver: true,
  ...
  return (
    ...
    >
      ...
      <SafeAreaView style={styles.safeArea} edges={["bottom"]}>
        <Animated.View style={[styles.titleArea, { opacity: Animated.multiply(contentFade, switchFade) }]}>
          {mode === "idle" && (
            <>
        ...
        {!expanded && <Animated.View style={{ transform: [{ translateY: slideAnim }] }}>
          <ReAnimated.View
            layout={LinearTransition.duration(360).easing(REasing.out(REasing.cubic))}
            style={[styles.bottomCard, cardAnimStyle]}
          >
            <Animated.View style={[styles.modeBlock, { opacity: switchFade }]}>

              {mode === "signup" && step === 1 && <StepDots step={1} total={TOTAL_STEPS} />}
              ...
              {mode === "login" && (
                <>
                  <Field placeholder="Username or Email" keyboardType="email-address" />
                  <Field placeholder="Password" secureTextEntry />
                  <View ref={loginBtnRef}>
                    <TouchableOpacity style={styles.primaryBtn} activeOpacity={0.88} onPress={handleLogin}>
          ...
          <Animated.View style={{ flex: 1, opacity: switchFade }}>
            ...
            >
              ...
              <View style={styles.expandedTitleRow}>
                ...
                <View style={{ flex: 1 }}>
                  <Text style={styles.expandedTitle}>{STEP_TITLES[step - 1]}</Text>
              ...
              {step === 5 && <StreamingGrid connected={streaming} onToggle={toggleStreaming} />}
      ...
      <Modal visible={showLoginExpand} transparent animationType="none">
        <Animated.View style={{
          position: "absolute",
          left: loginExpandLayout.x, top: loginExpandLayout.y,
          width: loginExpandLayout.w, height: loginExpandLayout.h,
...
const styles = StyleSheet.create({
  ...
  vignette:  { ...StyleSheet.absoluteFillObject, backgroundColor: "rgba(10,0,25,0.35)" },
  safeArea:  { flex: 1, justifyContent: "flex-end" },

  // Title (above normal card)
  titleArea: { paddingHorizontal: 32, paddingBottom: 28 },
  title: {
    fontSize: 42, fontFamily: "Inter_900Black", fontWeight: "900",
    color: "#ffffff", letterSpacing: -1, lineHeight: 50,
  },
  ...
  bottomCard: {
    ...
    marginHorizontal: 8, marginBottom: -19,
  ...
  expandedContainer: {
    position: "absolute",
    right: 10, left: 10, top: EXPANDED_TOP, bottom: 40,
  },
  expandedCard: {
    flex: 1,
    borderRadius: 32,
  ...
  expandedTitle: { fontSize: 26, fontWeight: "900", color: "#fff", letterSpacing: -0.4, textAlign: "center" },
  expandedSub:   { fontSize: 13, color: "rgba(255,255,255,0.38)", marginTop: 3, textAlign: "center" },
  ...
  usernameHint: { fontSize: 12, color: "rgba(255,255,255,0.28)", textAlign: "center" },
  errorBox: {
    flexDirection: "row", alignItems: "center", gap: 6,
    backgroundColor: "rgba(232,0,15,0.08)",
    borderRadius: 10, borderWidth: 1, borderColor: "rgba(232,0,15,0.2)",
  ...
  drumDivider: { width: StyleSheet.hairlineWidth, backgroundColor: "rgba(255,255,255,0.1)", marginVertical: 18 },
  ...
  artistAvatar:          { width: 52, height: 52, borderRadius: 26, alignItems: "center", justifyContent: "center", borderWidth: 2, marginBottom: 3 },
  artistInitials:        { fontSize: 17, fontWeight: "900" },
  artistName:            { fontSize: 13, fontWeight: "700", color: "#fff", textAlign: "center" },
  artistGenre:           { fontSize: 11, color: "rgba(255,255,255,0.35)" },
  artistFollowBtn:       { marginTop: 4, paddingHorizontal: 18, paddingVertical: 6, borderRadius: 20, backgroundColor: "#AB00FF" },
  artistFollowBtnActive: { backgroundColor: "rgba(171,0,255,0.15)", borderWidth: 1, borderColor: "#AB00FF" },
  ...
  primaryBtnText: {
    fontSize: 17, fontWeight: "700", fontFamily: "Inter_900Black",
    color: "#1a0030", letterSpacing: 0.3,
  },
  secondaryBtn: {
    backgroundColor: "rgba(171,0,255,0.25)", borderRadius: 50,
    paddingVertical: 18, alignItems: "center",
    borderWidth: 1, borderColor: "rgba(171,0,255,0.4)",
  },
  secondaryBtnText: {
    fontSize: 16, fontWeight: "600", fontFamily: "Inter_900Black",
  ...
  switchText:    { textAlign: "center", color: "rgba(255,255,255,0.45)", fontSize: 13, paddingBottom: 4 },
