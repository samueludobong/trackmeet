...
export const chatStyles = StyleSheet.create({
  ...
  // No `gap` here â€” inter-message spacing is set per-row via marginTop so groups
  ...
  // panel always renders at a consistent size â€” the body text wraps within this
  ...
  // Input bar â€” no longer absolute, sits in document flow
  inputBar: {
    flexDirection: "row",
    alignItems: "flex-end",
    gap: 8,
  ...
  // Reply quote â€” nested *inside* the bubble above the body: left accent bar,
  ...
  // No accent bar on the detached card â€” it's a fully-rounded pill, so the body
  ...
  // Song reply â€” album-art thumb + title Â· artist on one detail line.
